...
class DigitalWallet:
    ...
    def deposit(self, amount):
        if amount <= 0:
            return self._flag_suspicious("Deposit", amount, "Negative/Zero amount")

        self.balance += amount
        self.transactions.append((time.time(), "Deposit", amount, "Normal"))
        return "Success"
    ...
    def transfer(self, target_account, amount):
        ...
        for t, t_type, t_amt, status in self.transactions[-3:]:
            if t_type == "Transfer" and t_amt == amount and (now - t) < 10:
                return self._flag_suspicious(
                    "Transfer",
                    amount,
    ...
    def _flag_suspicious(self, transaction_type, amount, reason):
        flagged_status = f"Suspicious Transaction Flagged: {reason}"
